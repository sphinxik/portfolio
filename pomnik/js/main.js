...
function closeActiveCardFilter(target) {
	...
	if (openedCardFilter.length > 0) {
		if (!$(target).is(openedCardFilter)
			&& openedCardFilter.has(target).length === 0
			&& !$(target).is(openedCardFilterList)
			&& openedCardFilterList.has(target).length === 0
		) {
...
if ($('#map').length > 0) {
	...
	function startYandexMap() {
		ymaps.ready(function () {
			...
			// метка центра карты
			centerPlacemark = new ymaps.Placemark(map.getCenter(), {
				iconContent: ''
			}, {
			...
			for (let i = 0; i < mapCoords.length; i++) {
				baloonsCollection.add(new ymaps.Placemark(mapCoords[i], {
					balloonContent: ''
				}, {
					iconLayout: 'default#image',
					iconImageHref: 'images/icons/map-mark.svg',
